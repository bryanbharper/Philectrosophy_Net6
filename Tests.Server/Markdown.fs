...
let properties = testList "Markdown (Property)" [
    ...
    <| fun url ->
        ...
        // assert
        Expect.equal result $"![equation](%s{url})"
    ...
    <| fun (NonNull url) prefix ->
        ...
        // assert
        Expect.equal result $"![equation](%s{prefix}%s{String.urlEncode url})"
    ...
    <| fun (NonNull url) ->
        ...
        // assert
        Expect.equal result $"![equation](%s{Markdown.codeCogsUrl}%s{String.urlEncode url})"
    ...
    <| fun (NonNull url) ->
        ...
        // assert
        Expect.equal result $"![equation](%s{Markdown.codeCogsUrlInline}%s{String.urlEncode url})"
    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                let pattern = $"(%s{patternText})"
                let converter = String.slugify
            
                let inputPattern x = $"Some stuff blah %s{x} and other stuff %s{x}"
